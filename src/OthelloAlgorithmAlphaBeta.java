	...
	private int maxValue(OthelloPosition position, int alpha, int beta, int depth) {
		...
		if (moves.size() == 0) {
			...
			if (position.getMoves().size() == 0) {
				// if neither of the players can move, the game is over; return heuristic value (score in this case)
				if (position.toMove() != maxPlayer)
					position.nextMove();
				return evaluator.evaluate(position);
			}
		...
		for (OthelloAction action: moves) {
			try {
				// create new position, apply the action (may throw exception)
				new_position = position.clone().makeMove(action);
	...
	private int minValue(OthelloPosition position, int alpha, int beta, int depth) {
		...
		if (moves.size() == 0) {
			...
			if (position.getMoves().size() == 1) {
				// if neither of the players can move, the game is over; return heuristic value
				if (position.toMove() != maxPlayer)
					position.nextMove();
				return evaluator.evaluate(position);
			}
